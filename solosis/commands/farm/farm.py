...
def single_command(command_to_exec, job_name, queue, time, cores, mem, **kwargs):
    ...
    bash_submit(job_runner, command_to_exec = command_to_exec, job_name=job_name, queue=queue, time=time, cores=cores, mem=mem)


def array_cmd(command_to_exec, job_name, queue, time, cores, mem, num_of_concurrrent=20, **kwargs):
    """
    Run a single command on the farm.
    """
...
@helpers.farm
@click.option("-n", "--notebook", required=True, type=str, help = 'Path to the notebook to run')
@click.option("-j", "--job_name", required=False, type=str, help = 'Name of the job', default="random_val") #random val to be implemented (from import random)
def run_ipynb(notebook, job_name, **kwargs):
    ...
    bash_submit(job_runner, command_to_exec = command_to_exec, job_name="notebook", **kwargs) #, queue=queue, time=time, cores=cores, mem=mem)
