...
tail -n +2 $samples_file | while IFS=, read -r sample; do
  echo "mkdir -p $VOY_DATA/$sample/cellbender-results | cellbender remove-background --cuda --input $VOY_DATA/$sample/cellranger-hl/outs/raw_feature_bc_matrix.h5 --output $VOY_DATA/$sample/cellbender-results/$sample-cb.h5 --total-droplets-included $DROPLETS" >> $commands_file
done
